...
def plot_comprehensive_metrics(metrics, save_path, domain_title):
    ...
    plt.ylim(0, 1.0)
    plt.ylabel('Score')
    plt.title(f'MCU-Quake Baseline\nDataset: {domain_title}')
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(SAVE_BASE, f"Comparison_1C_KDE_{time_str}")
    ...
    # ===================================================================
    # 4. MEMBANGUN 3 KDE SEKALIGUS (KOMPONEN Z / 1C)
    # ===================================================================
    logger.info("Membangun KDE STEAD (Z)...")
    emb_Z_stead = dataset.load_embedding_data(EMB_DIR_STEAD, "Embedding data, Z.json")
    kde_stead = utils.embedding_PDFs_1D(emb_Z_stead, source_list=['noise', 'le'])

    logger.info("Membangun KDE UUSS (Z)...")
    emb_Z_uuss = dataset.load_embedding_data(EMB_DIR_UUSS, "Embedding data, Z.json")
    kde_uuss = utils.embedding_PDFs_1D(emb_Z_uuss, source_list=['noise', 'le'])

    logger.info("Membangun KDE INDONESIA (Z)...")
    emb_Z_indo = dataset.load_embedding_data(EMB_DIR_INDO, "Embedding data, Z.json")
    kde_indo = utils.embedding_PDFs_1D(emb_Z_indo, source_list=['noise', 'le'])
    ...
    for i in tqdm(range(len(keys_list)), desc="Inference Laten (Z) & 3 KDE"):
        record_key = keys_list[i]
        record = test_data[record_key]

        try:
            # 5a. Potong Sinyal (komponen Z saja)
            Z_n = record[f"{CHANNEL}_noise"][-num_points:]
            Z_s = record[CHANNEL][:num_points]

            # 5b. Ekstraksi Fitur dari Model (HANYA DILAKUKAN 1 KALI)
            emb_n_1c = utils.latent_codes_1D(Z_n, embedding_model)
            emb_s_1c = utils.latent_codes_1D(Z_s, embedding_model)

            # 5c. Uji Jarak ke KDE STEAD
            p_n_stead, _, _ = utils.infer_1C_PDFs(emb_n_1c, kde_stead, "Kernel")
            p_s_stead, _, _ = utils.infer_1C_PDFs(emb_s_1c, kde_stead, "Kernel")

            # 5d. Uji Jarak ke KDE UUSS
            p_n_uuss, _, _ = utils.infer_1C_PDFs(emb_n_1c, kde_uuss, "Kernel")
            p_s_uuss, _, _ = utils.infer_1C_PDFs(emb_s_1c, kde_uuss, "Kernel")

            # 5e. Uji Jarak ke KDE INDO
            p_n_indo, _, _ = utils.infer_1C_PDFs(emb_n_1c, kde_indo, "Kernel")
            p_s_indo, _, _ = utils.infer_1C_PDFs(emb_s_1c, kde_indo, "Kernel")
    ...
    for domain_name, predictions in domains:
        ...
        fig = utils.plot_confusion(f"MCU-Quake,{domain_name},Indonesia(1C)", true_labels_name, matrix, metrics)
    ...
    logger.info("\n" + "="*50)
    logger.info("✅ Pengujian Komparasi 3 KDE (Z / 1C) Selesai.")
