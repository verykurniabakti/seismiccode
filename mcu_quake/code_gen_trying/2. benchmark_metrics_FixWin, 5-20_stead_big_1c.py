...
if __name__ == "__main__":
    #===================================================================
    # 1. KONFIGURASI PATH & OUTPUT (MODIFIKASI PATH KE 1C)
    #===================================================================
    ...
    SAVE_DIR = '/Volumes/Extreme SSD/mcu_quake_big_stead_output_1c' # Folder baru
    ...
    # Load Model & Embeddings (Tetap menggunakan PDF 3D untuk menjaga integritas model Wu)
    ...
    #===================================================================
    # 3. BIG DATA STREAMING INFERENCE (SKEMA 1-COMPONENT)
    #===================================================================
    with h5py.File(HDF5_PATH, 'r') as f:
        ...
        for i, (idx, row) in enumerate(tqdm(df.iterrows(), total=len(df), desc="STEAD 1.2M 1C")):
            try:
                ...
                #=======================================================
                # MODIFIKASI UTAMA: SKEMA 1-COMPONENT (ZERO-PADDED)
                #=======================================================
                # Indeks 0=E, 1=N, 2=Z. Kita buat E dan N menjadi array nol.
                z_component = sample_3c[:, 2] # Ambil kanal Z asli
                empty_channel = np.zeros_like(z_component) # Array nol
                
                # Latent Extraction (E dan N dikirim sebagai NOL)
    ...
    fig.savefig(os.path.join(SAVE_DIR, "Final_Confusion_Matrix_1C.jpg"), dpi=300)
    
    print(f"\n[SUKSES] Akurasi Akhir Populasi STEAD (1C): {metrics.get('accuracy (avg.)')}")