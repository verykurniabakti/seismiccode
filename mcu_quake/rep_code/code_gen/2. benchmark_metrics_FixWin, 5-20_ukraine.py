...
if __name__ == "__main__":
    ...
    # Pastikan nama variabel ini sesuai dengan yang dipanggil di os.path.join
    base_path = "/Volumes/Local Disk/Code_Git/S3_code/seismic/mcu_quake/rep_code"

    # Lokasi data Ukraine
    key_data_dir = "/Volumes/Local Disk/Code_Git/S3_code/seismic/mcu_quake/Benchmark_ Ukraine 3C_ test n1335 r100 sdb8-956"
    key_test_file = "Ukraine data, test n1335 r100 sdb8-956.json"

    """ Dataset Tag """ 
    DATA_TAG = "Ukraine"
    true_labels = ["NO", "QB", "LE"]

    # Pemanggilan yang sebelumnya menyebabkan error
    ...
    for i in tqdm(range(len(keys_list)), desc="Progress", position=0):
        ...
        # Ambil data Z (Vertikal) yang pasti ada
        Z_data = record["Z"][:num_points]
        Z_noise_data = record["Z"][-num_points:]

        # Gunakan pengecekan (try-except) atau default ke Z jika N/E tidak ada
        # Ini mencegah KeyError jika data hanya 1-komponen
        N_data = record.get("N", Z_data)[:num_points]
        E_data = record.get("E", Z_data)[:num_points]
        
        N_noise_data = record.get("N", Z_noise_data)[-num_points:]
        E_noise_data = record.get("E", Z_noise_data)[-num_points:]
